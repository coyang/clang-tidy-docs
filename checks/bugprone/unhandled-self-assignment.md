...
For the CERT alias, the [WarnOnlyIfThisHasSuspiciousField]{.title-ref}
option is set to [false]{.title-ref}.
...
When [true]{.title-ref}, the check will warn only if the container class
of the copy assignment operator has any suspicious fields (pointer, C
array and C++ smart pointer). This option is set to [true]{.title-ref}
by default.
:::
